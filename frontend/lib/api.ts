...
export type Improvement = { run_id: string; parent_version_id: string; status: string; reason_codes: string[]; parent_metrics: MetricMap; candidate_metrics: MetricMap; window_pass_rate: number; window_count: number; parameter_changes?: Record<string, unknown>[]; windows?: Record<string, unknown>[] };
export type ImprovementContext = { current_version: Version | null; parent_version_id: string | null; parent_experiment_id: string | null; parent_critique: Critique | null; candidates_generated: number; candidates_evaluated: number; candidates_promoted: number; candidates_rejected: number; latest_candidate_status: string | null; gate_thresholds_persisted: boolean; cycle_boundaries_persisted: boolean };
...
export type System = { finagent_version: string; database_path: string; database_identity?: string; database_exists: boolean; database_size_bytes: number; database_backend: "sqlite" | "postgresql"; database_connectivity: boolean; git_revision: string | null; experiment_count: number; configuration_version_count: number; critique_count?: number; experiment_memory_count?: number; candidate_evaluation_count?: number; latest_experiment_id: string | null; dataset_count: number; latest_dataset_refresh: string | null; data_providers: DataProvider[]; data_quality_warnings: number; database_status: string; database_integrity: string | null; latest_experiment_at: string | null; latest_validation_at: string | null; last_successful_run: string | null; frontend_version: string | null; enabled_modules: string[]; demo_mode: boolean };
...
export const api = {
  ...
  improvementContext: () => request<ImprovementContext>("/api/improvements/context"),
