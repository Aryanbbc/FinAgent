...
export function DataFetchForm({ providers }: { providers: DataProvider[] }) {
  ...
  return <section className="panel"><h2>Fetch historical data</h2><p className="subtle">Daily research data only. Auto prefers configured Twelve Data, then tries Yahoo Finance, Stooq, and a supplied local CSV after retryable failures. Every saved dataset records both requested and actual provider provenance.</p><form className="filters" onSubmit={submit}><select aria-label="Provider" value={provider} onChange={(event) => setProvider(event.target.value)}>{providers.map(item => <option key={item.provider} value={item.provider} disabled={item.provider !== "auto" && !item.available}>{providerLabel(item.provider)}{item.requires_credentials && !item.available ? " (backend key required)" : ""}</option>)}</select><input name="symbol" placeholder="Symbol search (e.g. AAPL)" required/><input name="start" type="date" required/><input name="end" type="date" required/><input name="source_path" placeholder="CSV path for local_csv"/><label className="pill"><input name="refresh" type="checkbox"/> Force refresh</label><button disabled={status === "loading"}>{status === "loading" ? "Fetching historical data…" : "Fetch dataset"}</button></form>{status === "loading" && <div className="skeleton-list compact"><i/><i/></div>}{status !== "idle" && status !== "loading" && <div className={`request-state ${status}`}><span>{status === "success" ? "Dataset ready" : "Fetch unavailable"}</span><p>{message}</p>{status === "error" && <button type="button" disabled={!lastInput} onClick={() => lastInput && fetchDataset(lastInput)}>Retry fetch</button>}</div>}</section>;
}
