...
import { candidateChangeGroups, candidateChangeSummary, humanCode, metric, promotionChecks, reasonExplanation } from "@/lib/improvement-display";
...
import candidateChanges from "./candidate-changes.module.css";
...
function CandidateCard({ candidate, selected, onOpen }: { candidate: Improvement; selected: boolean; onOpen: (candidate: Improvement) => void }) {
  ...
  const groups = candidateChangeGroups(changes(candidate));
...
function ChangeGroups({ groups, drawer = false }: { groups: ReturnType<typeof candidateChangeGroups>; drawer?: boolean }) {
  if (!groups.length) return <p className={candidateChanges.empty}>No persisted parameter delta</p>;
  return <div className={`${candidateChanges.groups} ${drawer ? candidateChanges.drawerGroups : ""}`}>
    {groups.map((group) => <div className={candidateChanges.group} key={group.label}><strong>{group.label}</strong>{group.rows.map((row) => <p key={`${group.label}-${row.label}`}><span>{row.label}</span><small>{row.previous} → {row.proposed}</small></p>)}</div>)}
  </div>;
}
...
function CandidateDrawer({ candidate, detail, status, error, onClose }: { candidate: Improvement | null; detail: Improvement | null; status: "idle" | "loading" | "ready" | "error"; error: string; onClose: () => void }) {
  ...
  const groupedChanges = candidateChangeGroups(changes(record));
  return <div className="candidate-drawer-backdrop" role="presentation" onMouseDown={onClose}><aside className="candidate-drawer" role="dialog" aria-modal="true" aria-label={`${candidate.run_id} candidate detail`} onMouseDown={(event) => event.stopPropagation()}>
    ...
    {status === "ready" && <>
      <section><h3>Parameter delta</h3><ChangeGroups groups={groupedChanges} drawer/><details className={candidateChanges.raw}><summary>View raw changes</summary><pre>{JSON.stringify(changes(record), null, 2)}</pre></details></section>
