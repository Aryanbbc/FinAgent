...
export function WalkForwardMatrix({ items }: { items: Item[] }) {
  ...
  return <div><div className="walk-forward-timeline">{items.map((item, index) => <button type="button" className={`walk-window ${selected === index ? "selected" : ""}`} onClick={() => setSelected(index)} key={index}><span className="walk-window-name">{String(item.asset)} · W{String(item.window_index ?? index + 1)}</span><span className="walk-bars"><i className="train-bar">TRAIN</i><i className="test-bar">TEST</i></span><small>{String(item.train_start).slice(0,10)} → {String(item.train_end).slice(0,10)} <b/> {String(item.test_start).slice(0,10)} → {String(item.test_end).slice(0,10)}</small></button>)}</div>{current && <div className="selected-window"><strong>Selected held-out window</strong><span>Sharpe {Number((current.metrics as Record<string, number>).sharpe_ratio).toFixed(2)} · Return {(Number((current.metrics as Record<string, number>).total_return) * 100).toFixed(2)}% · Drawdown {(Number((current.metrics as Record<string, number>).maximum_drawdown) * 100).toFixed(2)}%</span></div>}</div>;
}
