...
export function CandidateEvolutionChart({ items }: { items: { run_id: string; candidate_metrics: Record<string, number | null>; status: string }[] }) {
  const values = items.slice().reverse().map((item) => ({ candidate: item.run_id, sharpe: item.candidate_metrics.sharpe_ratio ?? 0, status: item.status }));
  return <div className="chart"><ResponsiveContainer width="100%" height={240}><BarChart data={values} margin={{ left: 8, right: 18, top: 8 }}><CartesianGrid strokeDasharray="3 3" stroke="#263348" /><XAxis dataKey="candidate" stroke="#8090a8" tick={{ fontSize: 10 }} /><YAxis stroke="#8090a8" label={{ value: "OOS Sharpe", angle: -90, position: "insideLeft", fill: "#91a0b8", fontSize: 11 }} /><Tooltip contentStyle={tooltip} /><Bar dataKey="sharpe" name="Out-of-sample Sharpe" fill="#58d6aa" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer></div>;
}
