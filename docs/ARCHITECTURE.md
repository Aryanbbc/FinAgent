...
```text
providers / CSV
  → normalization + data quality + deterministic cache + dataset registry
  → features + causal regime detector
  → direct strategy OR Technical → Regime → Strategy → Risk agents
  → cost-aware backtest + metrics + benchmark
  → Critic Agent + experiment memory
  → explicit controlled-improvement / explicit research validation
  → SQLite + Markdown reports
  → FastAPI service layer → Next.js workspace, CLI, Streamlit debug dashboard
```
...
The Next.js app never reads SQLite. Its typed client consumes only FastAPI responses and presents persisted data, bounded local controls, loading/error/empty states, and clear historical-research labels.
