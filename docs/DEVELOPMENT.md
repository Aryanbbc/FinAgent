...
Keep FastAPI routes thin, data/research computation typed and testable, and frontend interaction through `frontend/lib/api.ts`. Preserve V0.1–V0.8 configuration compatibility. Public deployment uses explicit origins and a persistent SQLite disk; see [deployment](DEPLOYMENT.md). Do not put credentials, broker behavior, live/paper trading, LLMs, RL, or automated code changes into this repository.
